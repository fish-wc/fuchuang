...
class Differential_Privacy_Client(fl.client.NumPyClient):
		...
		for e in range(self.conf["local_epochs"]):
			...
			for batch_id, batch in enumerate(self.train_loader):
				...
				if self.conf["dp"]:
					...
					# norm_scale = min(1, self.conf['C'] / (model_norm))  这里加了一个小数
					elli=0.01
					norm_scale = min(1, (self.conf['C']+elli )/ (model_norm+elli))
...
def main() -> None:
	...
	# Start client
	client = Differential_Privacy_Client(conf, global_model, train_datasets, eval_datasets, id=args.node_id).to_client()
