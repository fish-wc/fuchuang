...
# 导入自己定义的必要包
from . import model_save_load as msl # 从同一目录下导入

# 定义类别
classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')
diseases=["PathMNIST","DermaMNIST","OCTMNIST","PneumoniaMNIST","RetinaMNIST","BreastMNIST"
...
def show_images(myimages, true_labels, predicted_labels,num_images=25,save_name="show_images",extension='.png'):
    # 默认在figures目录下
    ...
    for i in range(num_images):
        ...
        plt.title(f"True:{true_class_name} ({true_labels[i]}),Predict:{predict_class_name}", fontsize=12)
    plt.tight_layout()
...
def show_image(image,predict_label,true_label,save_name='show_oneImage',extension='.png'):
    ...
    plt.title(f"Predict:{predict_class_name},label({predict_label}),True:{true_label_name},label({true_label})", fontsize=12)
    save_base='./figures/showFigure'
    save_name=save_name+extension
    save_path=os.path.join(save_base,save_name)
    plt.savefig(save_path,dpi=300)
...
# 保存json文件
def save_json(save_path,images_name,predicted_labels):
    if save_path:
...
def plot_category(save_name,values,extension='.png'):
    # 统计每个类别的数量
