...
class Mydataset_numpy_client(Dataset):
    ...
    def __getitem__(self, index):
        if(self.mode=="train"):     
            ...
            featuremap=np.load(os.path.join(self.dataset+"/train",self.png_label_list[index][0]))
        ...
        if(self.mode=="test"):
            ...
            featuremap=np.load(os.path.join(self.dataset+"/test", self.png_label_list[index][0]))
            new_featuremap=featuremap.copy()
            featuremap_tensor=torch.from_numpy(new_featuremap)
            featuremap_tensor=featuremap_tensor.type(torch.FloatTensor)
            return featuremap_tensor,torch.tensor(self.png_label_list[index][1])